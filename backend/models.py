...
class AffiliateTemplate(Base):
    ...
    )  # LEGACY: không còn sử dụng, giữ để không phải migrate dữ liệu cũ
    ...
    )  # ví dụ: "https://go.example/deep_link?url={target}&sub1={sub1}"
...
class ProductOffer(Base):
    ...
    )  # successful/pending/unregistered
    eligible_commission = Column(
        Boolean, default=False
    )  # true nếu approval=successful & campaign running
    source_type = Column(
        String, nullable=True
    )  # datafeed | promotions | top_products | manual
    affiliate_link_available = Column(
        Boolean, default=False
    )  # có aff_link/deeplink hợp lệ
    product_id = Column(
        String, index=True, nullable=True
    )  # id sản phẩm theo nguồn (nếu có)
...
class PriceHistory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    offer_id = Column(
        Integer, ForeignKey("product_offers.id"), index=True, nullable=False
    )
    price = Column(Float, nullable=True)
    currency = Column(String, default="VND")
    recorded_at = Column(
...
class Campaign(Base):
    ...
    user_registration_status = Column(
        String, index=True, nullable=True
    )  # NOT_REGISTERED/PENDING/APPROVED
