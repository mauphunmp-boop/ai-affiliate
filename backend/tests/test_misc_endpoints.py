...
def test_links_crud(client):
    ...
    # Update
    upd = dict(payload); upd["name"] = "Link Shopee A"
...
def test_api_configs_crud(client):
    # Upsert
    r_upsert = client.post("/api-configs/upsert", json={
        "name": "testprov", "base_url": "https://api.example.com", "api_key": "KEY", "model": "m1"
    })
    assert r_upsert.status_code == 200
    cfg = r_upsert.json(); cfg_id = cfg["id"]

    # List
    r_list = client.get("/api-configs")
    assert r_list.status_code == 200 and any(c["name"] == "testprov" for c in r_list.json())

    # Update by id
    r_put = client.put(f"/api-configs/{cfg_id}", json={
        "name": "testprov", "base_url": "https://api2.example.com", "api_key": "NEW", "model": "m2"
    })
...
def test_campaigns_summary_and_list_and_approved_merchants(client):
    ...
    r_sync = client.post("/ingest/campaigns/sync", json={
        "provider": "accesstrade", "statuses": ["running"], "only_my": True, "enrich_user_status": False
    })
...
def test_ingest_policy_and_settings_and_rotate(client):
    ...
    # Linkcheck config
    r_cfg = client.post("/settings/linkcheck/config", json={"linkcheck_mod": 2, "linkcheck_limit": 5})
...
def test_ai_endpoints_with_and_without_products(client):
    ...
    with TestingSessionLocal() as db:
        crud.upsert_offer_by_source(db, schemas.ProductOfferCreate(
            source="manual", source_id="seed-ai", merchant="tiki", title="SP AI",
            url="https://tiki.vn/a", affiliate_url=None, image_url=None, price=100000, currency="VND",
            campaign_id=None, source_type="manual", extra=None
        ))

    r_sug2 = client.post("/ai/suggest", params={"query": "giới thiệu", "provider": "groq"})
...
def test_export_template_and_campaign_description(client):
    ...
    assert r_tpl.status_code == 200
    assert r_tpl.headers.get("content-type", "").startswith("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def test_ingest_commissions_unified(client):
    ...
    client.post("/ingest/campaigns/sync", json={
        "provider": "accesstrade", "statuses": ["running"], "only_my": True, "enrich_user_status": False
    })
    r = client.post("/ingest/commissions", json={"provider": "accesstrade", "merchant": "tikivn"})
    assert r.status_code == 200
    body = r.json()
...
def test_delete_all_offers_endpoint(client):
    ...
    with TestingSessionLocal() as db:
        crud.upsert_offer_by_source(db, schemas.ProductOfferCreate(
            source="accesstrade", source_id="d1", merchant="tikivn", title="DF 1",
            url="https://tiki.vn/1", affiliate_url=None, image_url=None, price=1, currency="VND",
            campaign_id="CID", source_type="datafeeds", extra=None
        ))
        crud.upsert_offer_by_source(db, schemas.ProductOfferCreate(
            source="accesstrade", source_id="tp1", merchant="tikivn", title="TP 1",
            url="https://tiki.vn/2", affiliate_url=None, image_url=None, price=2, currency="VND",
            campaign_id="CID", source_type="top_products", extra=None
        ))
        crud.upsert_offer_by_source(db, schemas.ProductOfferCreate(
            source="manual", source_id="m1", merchant="tikivn", title="Manual 1",
            url="https://tiki.vn/3", affiliate_url=None, image_url=None, price=3, currency="VND",
            campaign_id="CID", source_type="manual", extra=None
        ))
