...
def test_catalog_promotions_list_and_delete_by_campaign(client):
    ...
    r_ing = client.post(
        "/ingest/promotions", json={"provider": "accesstrade", "merchant": "tikivn"}
    )
    ...
    if len(items) > 0:
        r_del = client.delete(
            "/offers", params={"category": "promotions", "campaign_id": cid}
        )
...
def test_catalog_commissions_list_and_delete_by_campaign(client):
    ...
    r_del = client.delete(
        "/offers", params={"category": "commissions", "campaign_id": "CAMP3"}
    )
    assert r_del.status_code == 200
    body = r_del.json()
    assert body.get("ok") is True
...
def test_delete_offers_from_promotions_by_campaign(client):
    ...
    r_del = client.delete(
        "/offers", params={"category": "offers", "campaign_id": "CAMPX"}
    )
    assert r_del.status_code == 200
    body = r_del.json()
    assert body.get("ok") is True
...
def test_delete_single_promotion_and_commission_by_id(client):
    ...
    with TestingSessionLocal() as db:
        ...
        c = crud.upsert_commission_policy(
            db,
            schemas.CommissionPolicyCreate(
                campaign_id="CAMPY",
                reward_type="sale",
