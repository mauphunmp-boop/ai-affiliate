...
def _insert_metric(name: str, value: float, session_id: str = 's1'):
    ...
    db = TestingSessionLocal()
    try:
        m = models.WebVitalMetric(name=name, value=value, rating='good', session_id=session_id)
        db.add(m)
        db.commit()
    finally:
        db.close()
...
def test_trends_basic(client):
    ...
    for i in range(5):
        ...
        time.sleep(0.002)
    res = client.get('/metrics/web-vitals/trends?window_minutes=10&buckets=4&names=LCP,CLS')
