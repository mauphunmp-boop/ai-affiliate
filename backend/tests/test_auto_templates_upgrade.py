...
def client():
    ...
    with TestingSessionLocal() as db:
        ...
        legacy = AffiliateTemplate(
            ...
            network="accesstrade",
            platform=None,
            template="https://legacy/?url={target}",
            default_params=None,
            enabled=True,
        )
        db.add(legacy)
...
def test_upgrade_legacy_record(client):
    ...
    r = client.post(
        "/aff/templates/auto-from-campaigns", json={"network": "accesstrade"}
    )
    assert r.status_code == 200, r.text
    body = r.json()
    # Vì legacy đã nâng cấp thành platform=shopee nên 'shopee' nằm trong skipped (đã tồn tại sau upgrade)
    assert "shopee" in body.get("skipped", []) or any(
        c.get("platform") == "shopee" for c in body.get("created", [])
    )
