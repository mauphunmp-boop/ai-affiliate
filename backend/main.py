...
def export_offers_excel(
    ...
    def _campaign_field_from_log(cid: str, field_name: str):
        ...
        return "API_MISSING"
    # Chuẩn hoá placeholder: đổi API_EMPTY -> NO_DATA; giữ nguyên API_MISSING
    def _normalize_placeholder(v: Any) -> Any:
        if v == "API_EMPTY":
            return "NO_DATA"
        return v
    def _is_placeholder(v: Any) -> bool:
        return v in ("API_EMPTY", "API_MISSING", "NO_DATA")
    ...
    for idx, c in enumerate(campaigns_all, start=1):
        ...
        # Link mô tả chỉ hiện nếu thực sự có HTML mô tả; placeholder không tính là mô tả
        has_desc = False
        if not _is_placeholder(_desc_raw):
            _desc_text = _strip_html(_desc_raw)
            has_desc = bool(_desc_text)
        df_campaigns_rows.append({
            ...
            "status": _sanitize_val(c.status or _normalize_placeholder(_campaign_field_from_log(cid, "status"))),
            "start_time": _sanitize_val(c.start_time),
            "end_time": _sanitize_val(c.end_time if c.end_time else _normalize_placeholder(_campaign_field_from_log(cid, "end_time"))),
            "category": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "category"))),
            "conversion_policy": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "conversion_policy"))),
            "cookie_duration": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "cookie_duration"))),
            "cookie_policy": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "cookie_policy"))),
            ...
            "scope": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "scope"))),
            "sub_category": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "sub_category"))),
            "type": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "type"))),
            "campaign_url": _sanitize_val(_normalize_placeholder(_campaign_field_from_log(cid, "url"))),
        })
    ...
    trans_campaigns = {
        ...
        "scope": "Phạm vi", "sub_category": "Danh mục phụ",
    ...
    with pd.ExcelWriter(
        ...
        if ws_prod is not None:
            _write_urls(ws_prod, df_products, ["url", "affiliate_url", "image_url"])  # thêm image_url
